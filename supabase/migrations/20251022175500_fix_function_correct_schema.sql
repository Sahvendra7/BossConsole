-- ============================================================================
-- Fix get_user_secrets_with_shared function with correct schema
-- Issue: Referenced sm.tags and sm.recovery_codes which don't exist
-- Fix:
--   1. Tags are in separate secret_tags table - use LEFT JOIN + jsonb_agg
--   2. Column is recovery_codes_encrypted not recovery_codes - decrypt it
--   3. Include twofa_secret in metadata
-- ============================================================================
...
COMMENT ON FUNCTION public.get_user_secrets_with_shared IS 'Returns user''s own secrets + secrets shared with them - Fixed with correct schema: tags from secret_tags table, decrypted recovery_codes';

-- Success message
DO $$
BEGIN
    RAISE NOTICE '✅ Fixed get_user_secrets_with_shared function with correct schema';
    RAISE NOTICE '   - Tags: Aggregate from secret_tags table using jsonb_agg';
    RAISE NOTICE '   - Recovery codes: Decrypt recovery_codes_encrypted and cast to JSONB';
    RAISE NOTICE '   - Metadata: Include twofa_secret field';
