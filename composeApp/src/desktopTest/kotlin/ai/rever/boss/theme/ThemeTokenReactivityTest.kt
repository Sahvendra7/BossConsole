 ...
 *
 * The default theme (dark) and Daylight (light) are used because their
 * textSecondary values differ; if the palettes ever converge on that token, pick
 * another pair rather than deleting the guard.
 */
class ThemeTokenReactivityTest {
    ...
    fun `panel scrollbar indicator color follows the active theme`() {
        val defaultColor = PanelScrollbarConfig.indicatorColor

        BossThemeController.select("daylight")
        val daylightColor = PanelScrollbarConfig.indicatorColor

        assertNotEquals(defaultColor, daylightColor, "PanelScrollbarConfig froze its color at class-load")
    ...
    fun `horizontal bar scrollbar indicator color follows the active theme`() {
        val defaultColor = HorizontalBarScrollbarConfig.indicatorColor

        BossThemeController.select("daylight")
        val daylightColor = HorizontalBarScrollbarConfig.indicatorColor

        assertNotEquals(defaultColor, daylightColor, "HorizontalBarScrollbarConfig froze its color at class-load")
    }

    @Test
    fun `unknown file icon color follows the active theme`() {
        val defaultColor = FileIcons.Colors.unknown

        BossThemeController.select("daylight")
        val daylightColor = FileIcons.Colors.unknown

        assertNotEquals(defaultColor, daylightColor, "FileIcons.Colors.unknown froze its color at class-load")
    }
