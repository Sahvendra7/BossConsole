...
class McpAgentTraceTest {
    ...
    private fun core(timeout: Long = 5000, handler: McpToolHandler): McpToolRegistryCore =
        McpToolRegistryCore(disabledFile = null, invokeTimeoutMs = timeout).apply {
            registerProvider(object : McpToolProvider {
                override val providerId = "trace-test"
                override fun tools() = listOf(McpToolDefinition(name = "trace", description = "test", handler = handler))
            })
        }

    @Test
    fun `invocation returns original result while storing sanitized success or failure`() = runBlocking {
    ...
    @Test
    fun `handler exception and deadline are traced without changing error contract`() = runBlocking {
        val failing = core { throw IllegalStateException("broken") }
        assertTrue(failing.invoke("trace", "{}").isError)
        assertEquals(TraceStatus.FAILURE, AgentTraceStore.events.value.single().status)
        AgentTraceStore.clear()
        val slow = core(timeout = 50) { awaitCancellation() }
        assertTrue(slow.invoke("trace", "{}").isError)
        assertEquals(TraceStatus.TIMEOUT, AgentTraceStore.events.value.single().status)
    }

    @Test
    fun `caller cancellation propagates and marks running trace cancelled`() = runBlocking {
        ...
        entered.await()
        assertEquals(TraceStatus.RUNNING, AgentTraceStore.events.value.single().status)
        call.cancelAndJoin()
        assertTrue(call.isCancelled)
        assertEquals(TraceStatus.CANCELLED, AgentTraceStore.events.value.single().status)
    }

    @Test
    fun `rejected tool is not recorded as an executed trace`() = runBlocking {
